...
def src_name(sec_key, sec, typ):
    ...
    if sec_key == 'SNAP':
        return '%2.2d:%s' % (
            sec,
            typ,
        )  #  could be int, e.g. 7, in SNsrc system
    else:
...
def check_ranges(coords, txt):
    ...
    assert abs(dmax - dmin) < 1.0e-9, 'IRREGULAR COORDS %s %f %f %12.3e' % ( txt, 
                dmin, dmax, dmax - dmin,)
...
def readCams(ifile, wanted_poll=None, get_vals=False, dbgcc=None, dbg=None):
    ...
    if wanted_poll == 'PM':
        wanted_poll = None  # no longer needed
    ...
    nlons = (
        int((xmax - xmin) / dx) + 1
    )  # +1 needed to cope with uneven longitude range
    ...
    for poll in used_polls:  #  'NOX',:
        ...
        for n in range(len(df)):  # with open(ifile) as f:
            ...
            ix = int((lon - xmin) / dx)
            iy = int((lat - ymin) / dy)
            assert ( ix < nlons and iy < nlats), 'OOPSXY %6.3f %6.3f %s %d %d %d %d' % (
    ...
    if dbgcc is not None:
        ...
        for src in srcs:
            ...
            for poll in used_polls:
                print(
                    '%12.4e' % (0.001 * srcEmis[poll][dbgcc][src]['sum']),
                    end='',
                )  # kt
            print()

    if wanted_poll is None:
        return srcEmis
    else:
        return srcEmis[poll]
