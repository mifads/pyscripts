...
for row in f.iterrows():
  ...
  if r.Spec=='NH3' or r.Spec=='NH4_f':
    f=14.0/17.0                       # get N fraction for NH3
    if r.Spec=='NH4_f': f = 14.0/18.0 # N frac for NH4

    sumProd += ( f* ( r.emis + r.fluxin ) )
    sumLoss += ( f* ( r.ddep + r.wdep + r.fluxout ) )
    sumInit += ( f* r.init )
    sumMass += ( f* r.sum_mass )

sumDiff = sumInit + sumProd - sumLoss - sumMass  # should be zero?
print('N %10.3e %10.3e %10.3e %10.3e %10.3e' % ( sumInit, sumProd,sumLoss, sumMass, sumDiff) )
print('Pcnt diff ', 100*sumDiff/(sumInit+sumProd) ) # is this what we are interested in?
