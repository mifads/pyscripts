...
# May 12th . changed longitude to lon in 1st. 2018-02-26 - added back full:
  ...
  for n, var in enumerate(variables):
     ...
     for key in var.keys():
       if key == 'name' : continue # already done
       if key == 'data' : 
         datvar[:,:] = var['data'][:,:]
...
if __name__ == '__main__':
  ...
  TestVar=dict( name='TEST1', units='uuu', data=data )
  ...
  variables= [   
     dict(name='Var1',units='ppb',long_name='test_variable with CF-illegal units',data=data3[0,:,:]), 
     dict(name='Var2',units='ug/m3',data=data3[1,:,:]), 
     dict(name='Var3',units='m s-1',data=data3[2,:,:])
  ]
